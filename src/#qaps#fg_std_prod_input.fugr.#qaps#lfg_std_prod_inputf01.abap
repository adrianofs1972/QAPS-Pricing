...
FORM fill_std_producao CHANGING cv_ok TYPE abap_bool
                                cv_message TYPE bapi_msg.

  SELECT *
    FROM /qaps/std_prd_h
    INTO TABLE @gt_std_prd.
...
*  check lines( gt_list ) = 0.
  REFRESH: gt_list,
           gt_listbox.
  SELECT id_std_producao, codigo, descricao
              FROM /qaps/std_prd_h
              INTO CORRESPONDING FIELDS OF TABLE @gt_listbox.

  SORT gt_listbox BY codigo ASCENDING.

  LOOP AT gt_listbox ASSIGNING FIELD-SYMBOL(<fs_listbox>).
    <fs_listbox>-index = sy-tabix.
    gs_value-key  = <fs_listbox>-index.
*    gs_value-codigo = <fs_listbox>-codigo.
    data(lv_codigo) = |{ <fs_listbox>-codigo ALPHA = OUt }|.
    CONDENSE lv_codigo NO-GAPS.
    gs_value-text = lv_codigo && ` ` && <fs_listbox>-descricao.

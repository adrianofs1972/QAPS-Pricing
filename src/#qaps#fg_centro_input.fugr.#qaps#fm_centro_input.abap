FUNCTION /qaps/fm_centro_input .
*"----------------------------------------------------------------------
*"*"Interface local:
*"  IMPORTING
*"     VALUE(IV_ACTION) TYPE  CHAR1
*"     VALUE(IS_DATA) TYPE  /QAPS/S_CENTRO OPTIONAL
*"  EXPORTING
*"     VALUE(ES_DATA) TYPE  /QAPS/S_CENTRO
...
*"----------------------------------------------------------------------
  ...
  CASE sy-ucomm.
    WHEN 'OK'.
      ...
      es_data-id_grp_planta = VALUE #( gt_grp_planta[ index = gs_grp_planta-index ]-id_grp_planta OPTIONAL ).

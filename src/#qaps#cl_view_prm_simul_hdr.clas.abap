class /QAPS/CL_VIEW_PRM_SIMUL_HDR definition
  public
...
CLASS /QAPS/CL_VIEW_PRM_SIMUL_HDR IMPLEMENTATION.


  METHOD merge_data_to_html.

    FIELD-SYMBOLS <fs> TYPE /qaps/s_simulacao.
    ...
    lv_line = '<table style="width:90%">'.
    add_to_html lv_line.

    DATA(lv_simulacao) = |{ <fs>-id_simulacao ALPHA = OUT } - { <fs>-descricao }|.

    lv_line = `<tr><td>Simulação:</td><td style="width:50%">` && lv_simulacao && `</td>`.
    add_to_html lv_line.
    IF NOT <fs>-cod_std_producao IS INITIAL.

      DATA(lv_std_producao) = |{ <fs>-cod_std_producao ALPHA = OUT } - { <fs>-dsc_std_producao }|.
      CONDENSE lv_std_producao.
      lv_line = `<td>Std Produção:</td><td>` && lv_std_producao && `</td></tr>`.
    ELSE.
      lv_line = `<td>Std Produção:</td><td>(Não Atribuído)</td></tr>`.
    ENDIF.
    add_to_html lv_line.

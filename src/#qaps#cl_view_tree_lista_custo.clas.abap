class /QAPS/CL_VIEW_TREE_LISTA_CUSTO definition
  public
  inheriting from /QAPS/CL_VIEW_SIMPLE_TREE_BASE
  final
  create public .

public section.

  methods SET_DISPLAY_DELETE
    importing
      !IV_DISPLAY_DELETE type ABAP_BOOL .
  methods GET_SELECTED_NODE_KEY
    returning
      value(RETURN) type /QAPS/ED_COD_LISTA_CUSTO
    raising
      /QAPS/CX_PRICING_ERROR .

  methods GET_SELECTED_NODE
    redefinition .
  methods UPDATE
    redefinition .
protected section.

  methods ADD_CHILD_NODE
    redefinition .
  methods CUSTOMIZE_TOOLBAR
    redefinition .
  methods FUNCTION_SELECTED
    redefinition .
  methods NODE_DOUBLE_CLICK
    redefinition .
private section.

  data MV_SHOW_DELETE type ABAP_BOOL .
ENDCLASS.



CLASS /QAPS/CL_VIEW_TREE_LISTA_CUSTO IMPLEMENTATION.


  METHOD add_child_node.

    FIELD-SYMBOLS <fs_line> TYPE /qaps/s_lista_header.
    DATA lv_icon TYPE tv_image.

    ASSIGN ir_line->* TO <fs_line>.

    lv_icon = <fs_line>-icon.

    mo_tree->add_node( EXPORTING node_key          = CONV #( <fs_line>-cod_lista_custo )
                                 relative_node_key = 'Root'
                                 isfolder          = ''
                                 text              = CONV #( <fs_line>-descricao )
                                 image             = lv_icon ).

    DATA(lv_xml) = /qaps/cl_serialization=>serialize( ir_line ).
    APPEND VALUE ts_line_key( key     = <fs_line>-cod_lista_custo
                              text    = |{ <fs_line>-cod_lista_custo } { <fs_line>-descricao }|
                              is_root = abap_false
                              content    = lv_xml ) TO mt_nodes.

    mo_tree->expand_node(
     EXPORTING
       node_key            = 'Root'    " Node key
*        expand_predecessors =     " 'X': Expand Predecessor of Node
*        expand_subtree      =     " 'X': Expand all Subsequent Nodes
*        level_count         =     " Number of Lower Levels to be Expanded
*      EXCEPTIONS
*        node_not_found      = 1
*        others              = 2
   ).
    IF sy-subrc <> 0.
*     MESSAGE ID sy-msgid TYPE sy-msgty NUMBER sy-msgno
*                WITH sy-msgv1 sy-msgv2 sy-msgv3 sy-msgv4.
    ENDIF.

  ENDMETHOD.


  METHOD customize_toolbar.

    mo_toolbar->add_button(
     EXPORTING
       fcode     = '&SEP'
       icon = ''
       butn_type = cntb_btype_sep
       text      = ''  ).                                   "#EC NOTEXT

    mo_toolbar->add_button(
      EXPORTING
        fcode     = 'REFRESH'
        icon      = icon_refresh
        butn_type = cntb_btype_button
        text      = ''
        quickinfo = 'Atualizar' ).                          "#EC NOTEXT

    CHECK mv_show_delete = abap_true.

    mo_toolbar->add_button(
      EXPORTING
        fcode     = 'DELETE'
        icon      = icon_delete
        butn_type = cntb_btype_button
        text      = ''
        quickinfo = 'Excluir Lista de Custo' ).

  ENDMETHOD.


  METHOD function_selected.

    IF fcode = 'REFRESH'.
      RAISE EVENT on_function_selected
        EXPORTING
          iv_function = fcode
          iv_xml_data = VALUE #( ).
      RETURN.
    ENDIF.

    mo_tree->get_selected_node(
      IMPORTING
        node_key                   =  DATA(lv_key)   " Key of Selected Node
*       EXCEPTIONS
*         control_not_existing       = 1
*         control_dead               = 2
*         cntl_system_error          = 3
*         failed                     = 4
*         single_node_selection_only = 5
*         others                     = 6
    ).

    DATA(ls_data) = VALUE #( mt_nodes[ key = lv_key ] OPTIONAL ).

    IF ( NOT ls_data IS INITIAL AND ls_data-key <> 'Root' ) .

      RAISE EVENT on_function_selected
        EXPORTING
          iv_function = fcode
          iv_xml_data = ls_data-content.
    ELSE.
      MESSAGE 'Nenhum item foi selecionado' TYPE 'S' DISPLAY LIKE 'E'.
    ENDIF.

  ENDMETHOD.


  method GET_SELECTED_NODE.

    mo_tree->get_selected_node(
      IMPORTING
        node_key                   =  DATA(lv_node_key)   " Key of Selected Node
      EXCEPTIONS
        control_not_existing       = 1
        control_dead               = 2
        cntl_system_error          = 3
        failed                     = 4
        single_node_selection_only = 5
        OTHERS                     = 6
    ).
    IF sy-subrc <> 0.
*     MESSAGE ID sy-msgid TYPE sy-msgty NUMBER sy-msgno
*                WITH sy-msgv1 sy-msgv2 sy-msgv3 sy-msgv4.
    ENDIF.

    IF NOT lv_node_key IS INITIAL.
      DATA(ls_node) = VALUE #( mt_nodes[ key = lv_node_key ] OPTIONAL ).
      return = ls_node-content.
    ENDIF.

  endmethod.


  method GET_SELECTED_NODE_KEY.

    mo_tree->get_selected_node(
      IMPORTING
        node_key                   =  DATA(lv_node_key)   " Key of Selected Node
      EXCEPTIONS
        control_not_existing       = 1
        control_dead               = 2
        cntl_system_error          = 3
        failed                     = 4
        single_node_selection_only = 5
        OTHERS                     = 6
    ).
    IF sy-subrc <> 0.
*     MESSAGE ID sy-msgid TYPE sy-msgty NUMBER sy-msgno
*                WITH sy-msgv1 sy-msgv2 sy-msgv3 sy-msgv4.
    ENDIF.

    IF NOT lv_node_key IS INITIAL.
      DATA(ls_node) = VALUE #( mt_nodes[ key = lv_node_key ] OPTIONAL ).
      return = ls_node-key.
    ELSE.
      RAISE EXCEPTION TYPE /qaps/cx_pricing_error
        EXPORTING
          message  = VALUE bapiret2( type       = 'E'
                                     message    = 'Nenhum item foi selecionado'      ).

    ENDIF.

  endmethod.


  METHOD NODE_DOUBLE_CLICK.
    CHECK node_key <> 'Root'.

    RAISE EVENT on_node_double_click
      EXPORTING
        iv_node_key = node_key
        iv_xml_data = mt_nodes[ key = node_key ]-content.

  ENDMETHOD.


  METHOD set_display_delete.
    mv_show_delete = iv_display_delete.
  ENDMETHOD.


  METHOD update.

    FIELD-SYMBOLS <ft> TYPE /qaps/t_lista_header." ANY TABLE.

    mo_tree->delete_all_nodes( ).

    add_root_node( iv_root_text = mv_root_text ).

    ASSIGN ir_data->* TO <ft>.

    LOOP AT <ft> REFERENCE INTO DATA(lr_line).
      add_child_node( lr_line ).
    ENDLOOP.

  ENDMETHOD.
ENDCLASS.

FUNCTION /QAPS/FM_PROCESSO_INPUT .
*"----------------------------------------------------------------------
*"*"Interface local:
*"  IMPORTING
*"     REFERENCE(IV_ACTION) TYPE  CHAR1
*"     REFERENCE(IS_DATA) TYPE  /QAPS/CUSTO_PRC OPTIONAL
*"  EXPORTING
*"     REFERENCE(ES_DATA) TYPE  /QAPS/CUSTO_PRC
...
*"----------------------------------------------------------------------
  ...
  IF iv_action = 'E'.
    gs_processo = CORRESPONDING #( is_data ).
  ENDIF.
  ...
  CASE sy-ucomm.
    WHEN 'OK'.
      es_data = CORRESPONDING #( gs_processo ).
    WHEN 'CANCEL'.
      CLEAR: es_data.

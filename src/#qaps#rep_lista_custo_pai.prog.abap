...
MODULE pai_1000 INPUT.
  CASE sy-ucomm.
    WHEN 'GERAR'.
      go_controller->criar_lista_custo( ).
    WHEN 'REPROC'.
      go_controller->reprocessar_lista_custo( ).
    WHEN 'FINALIZAR'.
      go_controller->finalizar_lista_custo( ).
    WHEN 'ABRIR'.
      go_controller->reabrir_lista_custo( ).
    WHEN 'EXPORTAR'.
      go_controller->exportar( ).
    WHEN 'EFETIVAR'.
      go_controller->efetivar( ).
    WHEN 'SHOW'.
    ...
    WHEN 'CLOSE'.
      go_controller->close_detail( ).
      gv_detail = abap_false.
    WHEN 'BACK'.

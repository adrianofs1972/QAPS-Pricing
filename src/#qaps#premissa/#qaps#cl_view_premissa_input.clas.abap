class /QAPS/CL_VIEW_PREMISSA_INPUT definition
  public
...
public section.
  ...
  events ON_PREMISSA_CHANGED_FINISHED
    exporting
      value(IT_CHANGED_DATA) type /QAPS/T_PREMISSA_CHANGED_DATA .
  ...
  methods SET_SESSION_DATA
    importing
      ...
      !IS_PREMISSA_HEADER type /QAPS/S_PREMISSA_HEADER
      !IS_ITEM type /QAPS/S_PREMISSA_ITEM
...
private section.
  ...
  methods MERGE_DATA
    importing
      !IT_DATA type /QAPS/T_PREMISSA_DISTRIB
...
CLASS /QAPS/CL_VIEW_PREMISSA_INPUT IMPLEMENTATION.
  ...
  METHOD create_dynamic_catalog.
    ...
    CALL FUNCTION 'LVC_FIELDCATALOG_MERGE'
      EXPORTING
        i_structure_name = '/QAPS/S_PREMISSA_DISTRIB'
      CHANGING
        ct_fieldcat      = lt_fcat.
...
*    break c060863.
    LOOP AT lt_fcat ASSIGNING FIELD-SYMBOL(<fs>).
      IF <fs>-parameter0 = 'PERCENTUAL' AND ms_premissa_header-id_parent IS INITIAL
        AND ms_item-oculto = abap_false and ms_item-id_item_matriz is INITIAL
...
*    BREAK
  ...
  METHOD data_changed_finished.
    ...
    DATA lt_changed_data TYPE /qaps/t_premissa_changed_data.
    ...
    LOOP AT et_good_cells INTO DATA(ls_good_cells).
      ...
      LOOP AT <ft> ASSIGNING <fs>.
        ...
        DATA(ls_changed_data) = VALUE /qaps/s_premissa_changed_data(
            id_distribuicao = <fv_id>
            periodo      = <fv_periodo>
    ...
    RAISE EVENT on_premissa_changed_finished
      EXPORTING
...
* Get values and their short texts for given domain
  ...
  METHOD get_layout.
    ...
    return-grid_title = 'Distribuição por Origem'.
...
*      ENDLOOP.
  ...
  METHOD merge_data.
    ...
    TYPES: BEGIN OF ts_premissa,
             id_distribuicao TYPE /qaps/ed_id_distribuicao,
           END OF ts_premissa.
    ...
    DATA lt_premissa TYPE TABLE OF ts_premissa.
    ...
    lo_sdescr_main ?= cl_abap_structdescr=>describe_by_name( p_name = '/QAPS/PREM_DISTR' ).
    ...
    lo_sdescr_append ?= cl_abap_structdescr=>describe_by_name( p_name = '/QAPS/S_PREMISSA_DISTRIB' ).
    ...
    lt_premissa = VALUE #( FOR wa IN it_data
                            ( id_distribuicao = wa-id_distribuicao ) ).

    SORT lt_premissa BY id_distribuicao ASCENDING.
    DELETE ADJACENT DUPLICATES FROM lt_premissa COMPARING id_distribuicao.
    ...
    LOOP AT lt_premissa INTO DATA(ls_var_input).

      DATA(ls_data) = it_data[ id_distribuicao = ls_var_input-id_distribuicao ].
      ...
      LOOP AT it_data INTO ls_data
        WHERE id_distribuicao = ls_var_input-id_distribuicao.
      ...
      ASSIGN COMPONENT 'MODALIDADE' OF STRUCTURE <fs_line> TO FIELD-SYMBOL(<fv_modalidade>).

      IF <fv_modalidade> = 'I' OR <fv_modalidade> = 'N' OR <fv_modalidade> = 'T'.
  ...
  METHOD RESET.
    ...
    mo_alv->set_gridtitle( i_gridtitle = `Distribuição por Origem` ).
  ...
  METHOD SET_DATA.
    ...
    FIELD-SYMBOLS <fs> TYPE /qaps/t_premissa_distrib.
  ...
  METHOD set_grid_title.
    ...
    mo_alv->set_gridtitle( i_gridtitle = `Distribuição por Origem - ` && iv_title ).
  ...
  METHOD set_session_data.
    ...
    ms_premissa_header = is_premissa_header.
    ms_item = is_item.
  ...
  METHOD toolbar.
    ...
    check ms_premissa_header-id_parent IS INITIAL
        AND ms_item-oculto = abap_false.

    APPEND VALUE stb_button(
        function  = '&DEL_TRANSFER'
        icon      = icon_delete_row
        quickinfo = 'Excluir Transferência'
        butn_type = 0
        text      = 'Excluir Transferência' ) TO e_object->mt_toolbar.

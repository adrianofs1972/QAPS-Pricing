class /QAPS/CL_VIEW_MATRIZ_INPUT definition
  public
...
public section.
  ...
  methods SET_SESSION_DATA
    importing
      ...
      !IS_HEADER type /QAPS/S_MATRIZ_HEADER
      !IS_SIMULACAO type /QAPS/S_MATRIZ_ABAST_SIMULACAO .
...
private section.
  ...
  methods MERGE_DATA
    importing
      !IT_DATA type /QAPS/T_MATRIZ_DISTRIB
...
CLASS /QAPS/CL_VIEW_MATRIZ_INPUT IMPLEMENTATION.
  ...
  METHOD CREATE_DYNAMIC_CATALOG.
    ...
    CALL FUNCTION 'LVC_FIELDCATALOG_MERGE'
      EXPORTING
        i_structure_name = '/QAPS/S_MATRIZ_DISTRIB'
      CHANGING
        ct_fieldcat      = lt_fcat.
...
*                   OR fieldname = 'ID_VAR_INPUT'
    ...
    LOOP AT lt_fcat ASSIGNING FIELD-SYMBOL(<fs>).
      IF ( <fs>-parameter0 = 'PERCENTUAL' and ms_header-id_parent is INITIAL )
...
*    BREAK
  ...
  METHOD data_changed_finished.
    ...
    DATA lt_changed_data TYPE /qaps/t_matriz_changed_data.
...
*    BREAK-POINT.
    ...
    LOOP AT et_good_cells INTO DATA(ls_good_cells).
      ...
      LOOP AT <ft> ASSIGNING <fs>.
        ...
        DATA(ls_changed_data) = VALUE /qaps/s_matriz_changed_data(
...
*      ENDLOOP.
  ...
  METHOD merge_data.
    ...
    TYPES: BEGIN OF ts_matriz_dst,
             id_distribuicao TYPE /qaps/matriz_dst-id_distribuicao,
           END OF ts_matriz_dst.
    ...
    DATA lt_matriz_dst TYPE TABLE OF ts_matriz_dst.
    ...
    lo_sdescr_main ?= cl_abap_structdescr=>describe_by_name( p_name = '/QAPS/MATRIZ_DST' ).
    ...
    lt_matriz_dst = VALUE #( FOR wa IN it_data
                            ( id_distribuicao = wa-id_distribuicao ) ).

    SORT lt_matriz_dst BY id_distribuicao ASCENDING.
    DELETE ADJACENT DUPLICATES FROM lt_matriz_dst COMPARING id_distribuicao.

    DATA(lv_lines) = lines( lt_components ).

    LOOP AT lt_matriz_dst INTO DATA(ls_var_input).
...
*    sort <ft> by cod_porto cod_cais.
  ...
  METHOD reset.
    ...
    mo_alv->set_gridtitle( i_gridtitle = `Distribuição Portuária` ).
  ...
  METHOD set_data.
    ...
    FIELD-SYMBOLS <fs> TYPE /qaps/t_matriz_distrib.
  ...
  method SET_GRID_TITLE.
    ...
    mo_alv->set_gridtitle( i_gridtitle = `Distribuição Portuária - ` && iv_title ).
